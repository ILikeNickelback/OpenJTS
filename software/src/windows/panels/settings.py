...
class Experiment_settings_window(WindowBase):
    ...
    def __init__(
        ...
        self._on_open_sequence(
            n=1, label="Sequence 1"
        )  # start by showing params for seq 1

        if bus:
            bus.subscribe("open_sequence_settings", self._on_open_sequence)

    def _t(self, name):
        return f"settings_{name}_{self.UUID}"
    ...
    def _load_params(self):
        ...
        dpg.set_value(
            self._t("baseline"),
            params.get("baseline_points", _DEFAULTS["baseline_points"]),
        )
        dpg.set_value(
            self._t("nbr_averages"),
            params.get("nbr_of_averages", _DEFAULTS["nbr_of_averages"]),
        )
        dpg.set_value(
            self._t("time_averages"),
            params.get(
                "time_between_averages_ms", _DEFAULTS["time_between_averages_ms"]
            ),
        )
        dpg.set_value(
            self._t("seq_ignored"),
            params.get("nbr_sequences_ignored", _DEFAULTS["nbr_sequences_ignored"]),
        )
        dpg.set_value(
            self._t("time_next_seq"),
            params.get("time_before_next_seq_ms", _DEFAULTS["time_before_next_seq_ms"]),
        )
